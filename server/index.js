...
if (!PASSPHRASE) {
  console.error('LEADERBOARD_PASSPHRASE env var is required — refusing to start.')
  process.exit(1)
}
...
// Constant-time compare so a public passphrase check doesn't leak length/prefix via timing.
...
app.post('/api/results', async (request, reply) => {
  const { passphrase, name, date, guesses, won } = request.body ?? {}

  if (!isValidPassphrase(passphrase)) {
    return reply.code(401).send({ error: 'invalid_passphrase' })
  }
  if (!isValidName(name) || !DATE_RE.test(date ?? '') || !isValidGuesses(guesses) || typeof won !== 'boolean') {
...
app.get('/api/leaderboard', async (request, reply) => {
  const { passphrase, name, date } = request.query ?? {}
